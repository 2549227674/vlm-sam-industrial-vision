...
*注：本项目开发周期较为紧凑（4天），采用分段架构解耦验证策略。L1/L2 层的 "零样本自动标注生成管线（范式C）" 已进行小规模成功跑通验证；为在大批量数据工况下压力极限测试整个系统的多线高并发与 C++ 边缘通信架构性能，此后阶段的数据来源采用公有场景化数据集（Kaggle / MVTec / VisA）以模拟量产状态下数据积累的稳定环境。本项目为课程设计作品。*
