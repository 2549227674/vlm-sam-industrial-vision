...
### 2.5 范式 D — 多轴承生产线 YOLO 并发监控

**三线并发监控（单模型共享，监控看板实时统计）：**
...
- 实时统计面板 + 控制区
  ![范式D 统计总览](图片/img_12.png)

- 实时检测快照（三线并发）
  ![范式D 实时快照](图片/img_13.png)

- 多路合并视图（独立窗口）
  ![范式D 合并视图](图片/img_14.png)

- 各生产线趋势监控（FPS / 推理时延 / 缺陷数 / 帧数）
  ![范式D 趋势监控](图片/img_15.png)
...
| D | 多轴承 YOLO 并发监控 + 实时看板 | 三线并发示例：22.7 FPS，平均推理 306~309ms，显存约 1.7GB |
